...
class GameScreen : AppCompatActivity() { //    region Initialization
	...
	override fun onCreate(savedInstanceState: Bundle?) {
		...
		setContentView(R.layout.activity_game_screen) //        changeBackground() // always start with default
		...
		refRoomFirestore = Firebase.firestore.collection(getString(R.string.pathRoom)) //region Other Thread - player info update
	...
	override fun onStart() {
		...
		val layoutManager = LinearLayoutManager(this) //        layoutManager.reverseLayout = true
		...
		gameStateListener = object : ValueEventListener {
			...
			override fun onDataChange(GameState: DataSnapshot) {
				if (GameState.value != null && activityExists) {
					...
					if (gameState == 4) { //                        getCardsAndDisplay() // dummy - check if really required to read cards again from firebase - looks like waste
						if (soundStatus) SoundManager.getInstance()
							.playSuccessSound() // soundSuccess.start()
...
//				findViewById<HorizontalScrollView>(R.id.horizontalScrollView1).foreground = ColorDrawable(ContextCompat.getColor(applicationContext, R.color.inActiveCard))
	...
	private fun resetVariables() {
		...
		bidTeamScore = 0
		bidder = 0
		nextValidBidder = 0 // bidingStarted = false   /// biding happened before check if it is required here
		...
		if (gameState == 5) {
			...
			for (i in 0 until nPlayers) {
				...
				if (j == bidder || (j == buPlayer1 && buFound1 != 0) || (j == buPlayer2 && buFound2 != 0)) {
					...
					findViewById<TickerView>(refIDMappedTextViewA[i]).text = "$bidTeamScore /$bidValue" //                    findViewById<TickerView>(refIDMappedTextView[i]).text = playerName(j) + "\n$emojiScore  $bidTeamScore /$bidValue"
				} else {
					findViewById<TickerView>(refIDMappedTextView[i]).text = playerName(j) // + " ${pointsList.sum() - bidTeamScore} /${scoreLimit - bidValue}"
					findViewById<TickerView>(refIDMappedTextViewA[i]).text = "${pointsList.sum() - bidTeamScore} /${scoreLimit - bidValue}" //                    findViewById<TickerView>(refIDMappedTextView[i]).text = playerName(j) + "\n$emojiScore  ${pointsList.sum() - bidTeamScore} /${scoreLimit - bidValue}"
				}
			}
			var tt1 = 0
			var tt2 = 0
	...
	private fun decideGameWinnerTeam7(bidTeamScoreFinal: Int, totalGamePoints: Int) {
		if (bidTeamScoreFinal >= bidValue) { // bidder team won case
			if (this::roundListener.isInitialized) refGameData.child("RO")
				.removeEventListener(roundListener)
			clearAllAnimation()
	...
	private fun countDownTimer(task: String, purpose: String = "start") {
		if (purpose == "start") { //            timeCountdown = time
			findViewById<ImageView>(R.id.closeGameRoomIcon).visibility = View.GONE
			progressbarTimer.progress = 100
			findViewById<ImageView>(R.id.closeGameRoomIcon).visibility = View.GONE
			progressbarTimer.visibility = View.VISIBLE
			textViewTimer.visibility = View.VISIBLE
			textViewTimer.text = "10s" //            progressbarTimer.startAnimation(AnimationUtils.loadAnimation(applicationContext,R.anim.anim_scale_infinite))
	...
	private fun validateSelfPlayedCard(view: View) {
		if (gameState == 5 && "p$playerTurn" == from && gameTurn != 8 && gameTurn != 0) { // dummy error chance - why just 8 and not 5 also ? Investigate later
			...
			if (gameTurn == 1 || cardsSuit[cardSelected] == trumpStart || cardsSuit.slice(cardsInHand as Iterable<Int>)
				...
				if (vibrateStatus) vibrationStart() //                toastCenter("${playerName(playerTurn)}, please play ${getSuitName(trumpStart)} card")
	...
	private fun endGameRound() { //        if(buFound1 == 2) write("BU1/s1",1)  // locking the partner by 1
		...
		if (nPlayers4) write("CT", mutableMapOf("p1" to cardsIndexLimit, "p2" to cardsIndexLimit, "p3" to cardsIndexLimit, "p4" to cardsIndexLimit)) //        write("SC/p$roundWinner", tablePoints + ptAll[roundWinner - 1]) // don't use round winner - player update for themself so use fromInt
	...
	private fun startNextRound() {
		...
		write("RO", mutableMapOf("T" to 1, "P" to fromInt, "R" to "")) //        centralText("Please play your next card",2000)
	}
	...
	private fun compareCardsForWinner(currentCard: Int, winnerCard: Int): Int {
		...
		if ((cSuit == trump && wSuit != trump) || ((cSuit == wSuit) && ((currentCard - winnerCard) >= 1 || (nPlayers7 && currentCard % 2 == 1 && winnerCard == currentCard - 1) // current card is odd & more than winner card
		...
		addListenerForSingleValueEvent(object : ValueEventListener {
			...
			override fun onDataChange(dataLoad: DataSnapshot) {
				...
				if (bidder == fromInt) {  // only to bidder
					...
					partnerSelectRV.layoutManager = LinearLayoutManager(this@GameScreen, LinearLayout.HORIZONTAL, false)
	...
	private fun startTrumpSelection() { //        textViewBidValue.textColor = ContextCompat.getColor(this, R.color.progressBarPlayer4)
			...
			override fun onDataChange(dataLoad: DataSnapshot) {
				if (dataLoad.value != null && activityExists) {
					if (playerTurn != (dataLoad.child("BT").value as Long).toInt()) {
						...
						} else if (!bidSpeak && bidingStarted && soundStatus) speak("${playerName(prevPlayerTurn)} passed", speed = 1f) //                        else if (soundStatus) SoundManager.getInstance().playUpdateSound() //

						textViewBidValue.text = bidValue.toString() //.toString() //show current bid value
						findViewById<TextView>(R.id.textViewBider).text = "Bid: " + playerName(bidder) //                        textViewBidValue.textColor = ContextCompat.getColor(this, R.color.font_yellow)
						...
						if (playerTurn == fromInt && (bidder != playerTurn || !bidingStarted)) {
							if (bidStatus == 1) { // show bid frame and ask to bid or pass
								...
								bidded = true //                                toastCenter(" Sorry $selfName \n You cannot bid anymore")
	...
	fun askToBid(view: View) {
		...
		if (!bidded) {
			...
			when (view.tag) {
				"pass" -> {
					write("Bid/BS/$from", 0) //                    write("Bid", mutableMapOf("BV" to bidValue, "BT" to nextTurn(fromInt),"BB" to fromInt,"BS" to mutableMapOf("p1" to 1,"p2" to 1,"p3" to 1,"p4" to 1)))
					centralText("You passed the biding chance !!  \n You cannot bid anymore", 2500)
				}
				"5" -> {
					min(bidValue+5, maxBidValue)
					write("Bid/BV/", min(bidValue+5, maxBidValue))
					write("Bid/BB", playerTurn)
				}
				"10" -> {
					write("Bid/BV/", min(bidValue+10, maxBidValue))
					write("Bid/BB", playerTurn)
				}
				"20" -> {
					write("Bid/BV/", min(bidValue+20, maxBidValue))
					write("Bid/BB", playerTurn)
				}
				"50" -> {
					write("Bid/BV/", min(bidValue+50, maxBidValue))
					write("Bid/BB", playerTurn)
				}
				"75" -> {
					write("Bid/BV/", min(bidValue+75, maxBidValue))
					write("Bid/BB", playerTurn)
				}
			} //            write("Bid/BT", nextTurn(fromInt))
...
//					findViewById<HorizontalScrollView>(R.id.horizontalScrollView1).foreground = ColorDrawable(ContextCompat.getColor(applicationContext, R.color.inActiveCard))
	...
	override fun onStop() {
		super.onStop()
		refGameData.child("GS").removeEventListener(gameStateListener) //        try {
		if (this::roundListener.isInitialized) refGameData.child("RO")
			.removeEventListener(roundListener) //        } catch (me: Exception) {
